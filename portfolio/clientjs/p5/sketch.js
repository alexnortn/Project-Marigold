...
let glyph = function (p) {
	...
	function rayTest2() {
		let springVector = createVector(aSpringVert[5].x, aSpringVert[5].y);
		let lenOffset = springVector.dist(mouseVector);
		let lenMag = 150;
		let rayVector = createVector();
	...
	function clockDisp(alphaOpa) {
		let interests = 8;
		let tempLoc = createVector();
		let theta = TAU / interests;
		let radLen = ( w < h ? w : h ) / 2.75;
		let fadeSpeed1 = 10;

		for (let i = 0; i < interests; i++) {
			let selection;
			let x = (radLen * sin(theta * i)) + center.x;
			let y = (radLen * cos(theta * i)) + center.y - 50; // 50 = relative y offset
	...
	function dashedCircle(x,y,opacity) {
		...
		let circleRad = 12;
		let pointAmount = 25;
		let theta1 = TAU / pointAmount;
		for (let j = 0; j < pointAmount; j++) {
			let x1 = (circleRad * sin(theta1 * j)) + x;
			let y1 = (circleRad * cos(theta1 * j)) + y;
			push();
				fill(0,0,0,opacity);
				ellipse(x1, y1,1,1);
			pop();
	...
	function displayPhys() {
			for(let i in aVerts) {
					...
					let aVertPos = createVector(aLockVert[i].x, aLockVert[i].y);
			...
			for(let i in aCounterVerts) {
					strokeWeight(scaleFactor);
					let aVertPos = createVector(aCounterLockVert[i].x, aCounterLockVert[i].y);
					let trans = map(aVertPos.dist(mousePos), 0, center.x/2, 100, 0);
					let strokeCol = color(255,0,0,trans);
					stroke(strokeCol);
					fill(strokeCol);
			...
			for(let i in nVerts) {
					strokeWeight(scaleFactor);
					let aVertPos = createVector(nLockVert[i].x, nLockVert[i].y);
					let trans = map(aVertPos.dist(mousePos), 0, center.x/2, 100, 0);
					let strokeCol = color(255,0,0,trans);
					stroke(strokeCol);
					fill(strokeCol);
	...
	function displayPhys1(x, y, alphaOpa1) {
			strokeWeight(scaleFactor);
			let locConverge = createVector(x,y);
			for(let i in aVerts) {
					let aVertPos = createVector(aSpringVert[i].x, aSpringVert[i].y);
			...
			for(let i in aCounterVerts) {
					strokeWeight(scaleFactor);
					let aVertPos = createVector(aCounterSpringVert[i].x, aCounterSpringVert[i].y);
					let trans = map(aVertPos.dist(locConverge), 0, center.x, 100, 0);
					let strokeCol = color(255,0,0,trans);
					stroke(strokeCol);
					fill(strokeCol);
			...
			for(let i in nVerts) {
					strokeWeight(scaleFactor);
					let aVertPos = createVector(nSpringVert[i].x, nSpringVert[i].y);
					let trans = map(aVertPos.dist(locConverge), 0, center.x, 100, 0);
					let strokeCol = color(255,0,0,trans);
					stroke(strokeCol);
					fill(strokeCol);
	...
	function hoverButton() {
		let fadeSpeed = 20;
		let mousey = createVector(dashButton.loc().x, dashButton.loc().y);
	...
	function hoverButton1() {
		...
		let element = getElement('cross-circle');
