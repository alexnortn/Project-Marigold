...
var p5Intro = function(p) {
  ...
  function clockDisp(alphaOpa) {
    var interests = 8;
    var tempLoc = p.createVector();
    var theta = p.TAU / interests;
    ...
    for (var i = 0; i < interests; i++) {
      var selection;
      var x = (radLen * p.sin(theta * i)) + center.x;
      var y = (radLen * p.cos(theta * i)) + center.y;
  ...
  function dashedCircle(x,y,opacity) {
    ...
    var circleRad = 12;
    var pointAmount = 25;
    var theta1 = p.TAU / pointAmount;
    for (var j = 0; j < pointAmount; j++) {
      var x1 = (circleRad * p.sin(theta1 * j)) + x;
      var y1 = (circleRad * p.cos(theta1 * j)) + y;
      p.push();
  ...
  */
  ...
  function interestDisp(i, x, y, opacity) {
    ...
    if (i > 4) y -= 3;
    if (i == 6) x += 45;
    var textOpacity = p.map(opacity, 0, 255, 0, 1);
  ...
  function hoverButton() {
   ...
   p.push();
      ...
      p.strokeWeight(1);
      if (timeOut == 0) {
        p.line (mousey.x, mousey.y, mousey.x, mousey.y - lineLoc);
        p.line (mousey.x, mousey.y, mousey.x, mousey.y + lineLoc);
      }
    p.pop();
  ...
  */
  ...
  function MakeButton(_x, _y, _buttonWidth) {
    ...
    this.display = function(_x, _y) {
      ...
      p.strokeWeight(2);
      p.line(xPos, yPos+2.5, xPos, yPos-2.5);
